...
- [#PRNUM](https://github.com/openmls/openmls/pull/PRNUM): `openmls_rust_crypto` now supports `MLS_256_DHKEMP384_AES256GCM_SHA384_P384`, and the leaf node capability defaults advertise it. The crate already had every primitive the suite needs; it was only missing from `supports` and `supported_ciphersuites`.
